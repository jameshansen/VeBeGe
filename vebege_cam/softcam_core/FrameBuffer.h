...
/// One buffer per slot (per virtual camera).
class FrameBuffer
{
 public:
    ...
    static FrameBuffer open(int slot);
    ...
    void            deactivate();
    /// Receiver liveness: the DirectShow filter bumps this every FillBuffer
    /// while an app is actually streaming; the sender watches it to know
    /// when it can release the physical camera.
    void            bumpReceiverHeartbeat();
    uint8_t         receiverHeartbeat() const;
