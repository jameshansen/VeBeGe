...
// Per-slot kernel object names (one virtual camera per slot).
const char* mutexName(int slot)
{
    static char buff[MAX_SLOTS][64];
    std::snprintf(buff[slot], sizeof(buff[slot]), "VeBeGe/NamedMutex.%d", slot);
    return buff[slot];
}

const char* sharedMemoryName(int slot)
{
    static char buff[MAX_SLOTS][64];
    std::snprintf(buff[slot], sizeof(buff[slot]), "VeBeGe/SharedMemory.%d", slot);
    return buff[slot];
}

bool validSlot(int slot) { return 0 <= slot && slot < MAX_SLOTS; }
...
struct FrameBuffer::Header
{
    ...
    uint8_t     m_receiver_heartbeat;   // bumped by the filter while streaming
...
FrameBuffer FrameBuffer::create(
                        ...
                        float           framerate)
{
    FrameBuffer fb(mutexName(validSlot(slot) ? slot : 0));
    ...
    fb.m_shmem = SharedMemory::create(sharedMemoryName(slot), shmem_size);
    if (fb.m_shmem)
    {
...
FrameBuffer FrameBuffer::open(int slot)
{
    FrameBuffer fb(mutexName(validSlot(slot) ? slot : 0));
    ...
    fb.m_shmem = SharedMemory::open(sharedMemoryName(slot));
    if (fb.m_shmem)
    {
        std::lock_guard<NamedMutex> lock(fb.m_mutex);
...
void FrameBuffer::bumpReceiverHeartbeat()
{
    if (!m_shmem) return;
    std::lock_guard<NamedMutex> lock(m_mutex);
    header()->m_receiver_heartbeat += 1;
}

uint8_t FrameBuffer::receiverHeartbeat() const
{
    std::lock_guard<NamedMutex> lock(m_mutex);
    return m_shmem ? header()->m_receiver_heartbeat : 0;
}
