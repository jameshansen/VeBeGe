...
#include <memory>
#include <softcam_baseclasses/streams.h>
...
private:
    ...
    Softcam(LPUNKNOWN lpunk, const GUID& clsid, int slot, HRESULT *phr);
};
...
private:
    ...
    std::unique_ptr<uint8_t[]>  m_placeholder;   // "VeBeGe" placeholder image
    ...
    Softcam*        getParent();
    void            fillPlaceholder(BYTE* pData); // shown when no sender is running
};
